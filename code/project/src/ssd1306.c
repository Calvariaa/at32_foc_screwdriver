...
uint8_t ssd1306_Init(void) {
  ...
  // i2c_transmit_set(hi2c1.i2cx, SSD1306_I2C_ADDR, 0, I2C_AUTO_STOP_MODE, I2C_GEN_START_WRITE);
  // if (i2c_wait_flag(&hi2c1, I2C_ADDRF_FLAG, I2C_EVENT_CHECK_ACKFAIL, 0x0002FFFF) != I2C_OK) {
  //   SSD1306.Initialized = 0;
  //   /* Return false */
  //   printf("init ssd1306 failed\n");
  //   return 1;
  // }
...
void ssd1306_WriteFloat(const double dat, uint8_t num, uint8_t pointnum, FontDef Font) {
  ...
  if (SSD1306.CurrentX + (num + pointnum + 2) * Font.FontWidth < 127)
...
void ssd1306_WriteChinese(uint8_t x, uint8_t y, uint8_t *data, uint8_t num) {
  for (int i = 0; i < num; i++) {
    ssd1306_DrawBitmap(x + i * 16, y, 16, 26, data + i * (64));
  }
}
...
void ssd1306_WriteCommand(uint8_t command) {
  i2c_memory_write(&SSD1306_I2C_PORT, I2C_MEM_ADDR_WIDIH_8, SSD1306_I2C_ADDR, 0x00, &command, 1, 0x0002FFFF);
}

void ssd1306_WriteData(uint8_t *data, uint16_t size) {
  i2c_memory_write(&SSD1306_I2C_PORT, I2C_MEM_ADDR_WIDIH_8, SSD1306_I2C_ADDR, 0x40, data, size, 0x000FFFFF);
}
...
void ssd1306_WriteCommand(uint8_t command) {
  ...
  while (SSD1306_I2C_PORT.status != 0) {
  };
  i2c_command = command;
  // HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x00, 1, &i2c_command, 1);
  i2c_memory_write_dma(&SSD1306_I2C_PORT, I2C_MEM_ADDR_WIDIH_8, SSD1306_I2C_ADDR, 0x00, &command, 1, 0x0002FFFF);
}
...
void ssd1306_UpdateScreen(void) {
  if (ssd1306_updatestatus == 0) {
    ...
    i2c_command = 0xB0;
    i2c_memory_write_dma(&SSD1306_I2C_PORT, I2C_MEM_ADDR_WIDIH_8, SSD1306_I2C_ADDR, 0x00, &i2c_command, 1, 0x0002FFFF);
...
void ssd1306_mem_cplt_callback() {
  uint32_t phase;
  if (ssd1306_updatestatus) {
    if (ssd1306_updatestatus < ssd1306_updateend) {
      ...
      if (phase == 3)
        i2c_memory_write(&SSD1306_I2C_PORT, I2C_MEM_ADDR_WIDIH_8, SSD1306_I2C_ADDR, 0x40, &SSD1306_Buffer[
        ...
        i2c_memory_write_dma(&SSD1306_I2C_PORT, I2C_MEM_ADDR_WIDIH_8, SSD1306_I2C_ADDR, 0x00, &i2c_command, 1,
                             0x0002FFFF);
      }
    } else {
